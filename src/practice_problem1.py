...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if space > len(additional_contents):
            space = len(additional_contents)

        str_to_add = ''
        for i in range(space):
            str_to_add = str_to_add + additional_contents[i]

        self.contents = self.contents + str_to_add

        str_to_return = ''
        for i in range(space, len(additional_contents)):
            str_to_return = str_to_return + additional_contents[i]
    ...
    def shrink(self, new_volume):
        ...
        # and THEN translate the pseudo-code to a solution.
        # --------------------------------------------------------------
        length_contents_to_cut = len(self.contents) - new_volume

        str_to_return = ''

        if length_contents_to_cut > 0:

            index_to_start = len(self.contents) - length_contents_to_cut

            for i in range(index_to_start, len(self.contents)):
                str_to_return = str_to_return + self.contents[i]

            new_contents = ''
            for i in range(index_to_start):
                new_contents = new_contents + self.contents[i]
    ...
    def double_then_shrink(self, new_volume):
        ...
        # --------------------------------------------------------------
        total_cut = 0
        cut_double = self.double()
        total_cut = total_cut + len(cut_double)

        cut_shrink = self.shrink(new_volume)
        total_cut = total_cut + len(cut_shrink)
